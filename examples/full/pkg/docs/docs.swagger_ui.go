...
func Swagger() (p string, handler http.HandlerFunc) {
    ...
    mux.Handle(BasePath, swaggerui.NewHandler(
        ...
        swaggerui.WithSpecURLs("BasicService",
            []swaggerui.SpecURL{ 
            {
                Name: "BasicService",
                URL:  path.Join(swaggerPath, "basic_service.swagger.json"),
            },
            {
                Name: "MatreshkaCommon",
                URL:  path.Join(swaggerPath, "matreshka_common.swagger.json"),
            },
            {
                Name: "MatreshkaApi",
